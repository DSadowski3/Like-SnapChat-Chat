...
- (void)viewDidLoad
{
    ...
    [self.delegate shoudlEnableCameraUI:NO];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
	if ( context == CapturingStillImageContext ) {
		...
		dispatch_async( dispatch_get_main_queue(), ^{
            ...
            [self.delegate shoudlEnableCameraUI:isSessionRunning && ([AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo].count >1)];
...
- (void)changeCamera
{
	...
	dispatch_async( self.sessionQueue, ^{
		...
		dispatch_async( dispatch_get_main_queue(), ^{
            ...
            [self.delegate shoudlEnableCameraUI:YES];
...
- (void)captureOutput:(AVCaptureFileOutput *)captureOutput didFinishRecordingToOutputFileAtURL:(NSURL *)outputFileURL fromConnections:(NSArray *)connections error:(NSError *)error
{
	...
	if ( error ) {
        ...
        [self.delegate videoRecordingFaild];
	}
	if ( success ) {
	...
	dispatch_async( dispatch_get_main_queue(), ^{
		// Only enable the ability to change camera if the device has more than one camera.
        [self.delegate shoudlEnableCameraUI: [AVCaptureDevice devicesWithMediaType:AVMediaTypeVideo].count > 1 ];
        [self.delegate shouldEnableRecordUI:YES];
